        ...
        <div class="container option">
            <div class="row mt-5">
                    ...
                    <div class="dropdown-danhsach">
                        ...
                        <ul class="dropdown-content" style="list-style-type: inherit;">
                            @foreach ($specials as $row)
                                <li class="special-type" data-special="{{ $row->id }}"
                                    style="{{ $row->id == app('request')->input('product-special') ? 'background-color:lightgray' : '' }}">
                                    {{ $row->name }}</li>
                            @endforeach
                        </ul>
                    </div>
                    ...
                    <div class="dropdown-danhsach">
                        ...
                        <ul class="dropdown-content" style="list-style-type: inherit;">
                            @foreach ($productType as $row)
                                <li class="product-type" data-type="{{ $row->id }}"
                                    style="{{ $row->id == app('request')->input('product-type') ? 'background-color:lightgray' : '' }}">
                                    {{ $row->name }}</li>
                            @endforeach
                        </ul>
                    </div>
                ...
                <div class="col-md-9">
                    <div class="row">
                        <div class="col-md-12">
                            <img src="https://rsggroup.com/wp-content/uploads/2020/03/bg-3-980x325.jpg"
                                class="img-thumbnail">
                        </div>
                    </div>
                    <div class="row">
                        <div class="search-bar col-12">
                            <div>
                                Tăng dần <input type="radio" class="sort" value="1"
                                    {{ app('request')->input('sort') == '1' ? 'checked' : '' }}>
                                Giảm dần <input type="radio" class="sort" value="0"
                                    {{ app('request')->input('sort') == '0' ? 'checked' : '' }}>
                            </div>
                            <form>
                            ...
                            <div class="col-md-4">
                                <div class="border-product">
                                    <img class="img-thumbnail"
                                        src="{{ asset('images/' . $product->component->first()->image ?? null) }}">
