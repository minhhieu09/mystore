...
class CustomerController extends Controller
{
    ...
    public function Info(Request $request){
        $dataRequest = $request->all();
        dd($dataRequest);
        $user = User::find($dataRequest['id'])->first();

        $user->first_name = $dataRequest['first_name'];
        $user->last_name = $dataRequest['name'];
        $user->distric_name = $dataRequest['distric_name'];
        $user->city_name = $dataRequest['city_name'];
        $user->number = $dataRequest['number'];

        $user->save();

        return redirect()->back()->with(['status' => 'successs', 'message' => 'Cập nhật thông tin thành công']);
    }
}
