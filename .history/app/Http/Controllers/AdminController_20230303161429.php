...
class AdminController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $assign['type'] = ProductTypeModel::where('status', 1)->all();
        $assign['memory'] = [
            '64' => '64GB',
            '128' => '128GB',
            '512' => '512GB'
        ];
        $assign['color'] = ProductsColorModel::where('status', 1)->all();
        $assign['products'] = $assign['productComponent']->product;
        $assign['id'] = $id;
    ...
    public function store(Request $request)
    {
        ...
        if (isset($dataRequest['id'])) {
            ...
            ProductComponentModel::find($dataProductComponent, $dataRequest['id']);
            foreach($dataProductComponent as $key)
    ...
    public function delete($id)
    {
        ...
        if (ProductComponentModel::where('product_id', 'id')->count() == 1) {
            ProductsModel::find($id)->delete($product->id);
            $product->special()->where('product_id', $product->id)->detach();
        }

        ProductComponentModel::find($id)->delete($id);
